...
function formatSupabaseError(error: unknown): { code?: string; message: string } {
...
export async function upsertPromptStats(
    stats: Array<{ prompt_id: string; views: number; copies: number; likes: number }>
...
export async function checkSupabaseTable(tableName: 'prompt_stats' | 'stat_events'): Promise<SupabaseTableHealth> {
    if (!supabase) {
        return {
            ok: false,
            count: null,
...
export async function getRecentStatEvents(limit = 5): Promise<Array<{ prompt_id: string; event_type: string; created_at: string }>> {
    if (!supabase) return []

    const { data, error } = await supabase
        .from('stat_events')
        .select('prompt_id,event_type,created_at')
