...
import { getAllCategories, sortPrompts, paginatePrompts, getTagBySlug } from "@/lib/data-utils"
import type { Category, Prompt, SortBy, Tag } from "@/lib/types"
import { useSearchParams } from "next/navigation"

const validSorts: SortBy[] = ["latest", "popular", "trending", "copies", "likes"]
...
export function ClientGallery({ initialPrompts, categories: initialCategories, tags, periodStats }: ClientGalleryProps) {
    const searchParams = useSearchParams()
    const categoryParam = searchParams.get("category")
    const tagParam = searchParams.get("tag")
    const sortParam = searchParams.get("sort") as SortBy | null

    const [sortBy, setSortBy] = useState<SortBy>(sortParam && validSorts.includes(sortParam) ? sortParam : "latest")
    ...
    const categories = getAllCategories(initialCategories)
    ...
    if (tagParam) {
        const tag = getTagBySlug(tags, tagParam)
        if (tag) {
        ...
        <div className="min-h-screen bg-background">
            <main className="w-full max-w-[2400px] mx-auto px-3 py-6">
                ...
                <section className="text-center py-8 md:py-12">
                    ...
                    <PopularTags prompts={allPrompts} tags={tags} className="mt-5" />
                </section>
