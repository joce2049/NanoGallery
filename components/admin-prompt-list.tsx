...
import { getPromptPreviewUrl } from "@/lib/utils"
                    ...
                    <div key={prompt.id} className="p-4 flex items-center gap-4 hover:bg-accent/50 transition-colors group">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                <span className={`text-xs px-2 py-0.5 rounded-full border ${prompt.status === 'published'
                                    ...
                                    {prompt.status === 'published' ? '已发布' : prompt.status === 'archived' ? '已归档' : '草稿'}
                                </span>
                        ...
                        <div className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                            <Button
                                ...
                                disabled={updatingId === prompt.id || prompt.status === "archived"}
                            >
                                {prompt.status === "published" ? "转草稿" : "发布"}
                            </Button>
                            <Button
                                size="icon"
                                variant="ghost"
