...
import { Copy, X, Heart, Sparkles, Terminal, Share2, Check, ChevronLeft, ChevronRight } from "lucide-react"
import type { Prompt, Tag } from "@/lib/types"
import { getPromptTags, recordCopy, recordLike } from "@/lib/data-utils"
...
export function ImageModal({ prompt, open, onOpenChange, onSelectPrompt, allPrompts = [], tags = [], onStatsChange }: ImageModalProps) {
    ...
    const currentIndex = prompt ? allPrompts.findIndex(item => item.id === prompt.id) : -1
    ...
    const promptTags = getPromptTags(prompt, tags)
    ...
    const handleCopyPrompt = async (e?: React.MouseEvent) => {
        ...
        try {
            ...
            if (success) {
                ...
                onStatsChange?.(prompt.id, nextStats)
                setCopied(true)
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            >
                <div className="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_360px] lg:grid-cols-[minmax(0,1fr)_420px] grid-rows-[38vh_minmax(0,1fr)] md:grid-rows-1 h-full min-h-0 w-full overflow-hidden">
                    ...
                    <div className="flex flex-col border-t md:border-t-0 md:border-l border-border/70 min-h-0 bg-background/90 backdrop-blur-xl h-full overflow-hidden">
                        ...
                        <div className="flex-1 min-h-0 overflow-hidden">
                            <div className="h-full p-4 md:p-8 flex flex-col gap-4 md:gap-6 overflow-hidden">
                                ...
                                <div className="space-y-4 shrink-0">
                                    ...
                                    {prompt.description && (
                                        <p className="text-muted-foreground text-sm md:text-base leading-relaxed line-clamp-2">{prompt.description}</p>
                                    )}
                                </div>
                        ...
                        {/* Sticky Footer */}
                        <div className="p-3 md:p-6 border-t border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 flex flex-col gap-3 shrink-0">
                            <Button
                                size="lg"
